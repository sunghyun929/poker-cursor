...
export class HandEvaluator {
  ...
  private getSuitValue(suit: Card['suit']): number {
    const values: Record<Card['suit'], number> = {
  ...
  private checkRoyalFlush(cards: Card[]): HandRank | null {
  ...
  private checkStraightFlush(cards: Card[]): HandRank | null {
    ...
    for (const rank in rankGroups) {
      if (rankGroups[rank].length >= 4) {
        ...
        return {
          rank: 8,
          description: "포카드",
          cards: [...fourCards, ...kickers], // Return all 5 cards
          kickers: []
        };
      }
    }
  ...
  private checkFullHouse(cards: Card[]): HandRank | null {
    ...
    // Find all triplets and pairs
    for (const rank in rankGroups) {
      if (rankGroups[rank].length >= 3) {
        triplets.push(rankGroups[rank].slice(0, 3));
      } else if (rankGroups[rank].length >= 2) {
        pairs.push(rankGroups[rank].slice(0, 2));
      }
    }

    // Sort triplets by rank value (highest first) for proper full house ranking
    triplets.sort((a, b) => this.getRankValue(b[0].rank) - this.getRankValue(a[0].rank));
    pairs.sort((a, b) => this.getRankValue(b[0].rank) - this.getRankValue(a[0].rank));

    if (triplets.length >= 1) {
      const threeOfAKind = triplets[0];
      let pair: Card[] | null = null;

      // Check if we have a second triplet (can use as pair)
      if (triplets.length >= 2) {
        pair = triplets[1].slice(0, 2);
      } else if (pairs.length >= 1) {
        // Find highest pair that's not the same rank as triplet
        for (const pairGroup of pairs) {
          if (pairGroup[0].rank !== threeOfAKind[0].rank) {
            pair = pairGroup;
            break;
          }
        }
      ...
      if (pair) {
        return {
          rank: 7,
          description: "풀하우스",
          cards: [...threeOfAKind, ...pair], // Three-of-a-kind first for proper comparison
          kickers: []
        };
      }
    }
    ...
    for (const rank in rankGroups) {
      if (rankGroups[rank].length >= 3) {
        ...
        return {
          rank: 4,
          description: "트리플",
          cards: [...threeCards, ...kickers], // Return all 5 cards
          kickers: []
        };
      }
    }
  ...
  private checkTwoPair(cards: Card[]): HandRank | null {
    ...
    if (pairs.length >= 2) {
      ...
      return {
        rank: 3,
        description: "투 페어",
        cards: [...twoPairCards, ...kickers], // Return all 5 cards
        kickers: []
      };
    }
    return null;
    ...
    for (const rank in rankGroups) {
      if (rankGroups[rank].length >= 2) {
        ...
        return {
          rank: 2,
          description: "원 페어",
          cards: [...pairCards, ...kickers], // Return all 5 cards
          kickers: []
        };
      }
    }
  ...
  private checkHighCard(cards: Card[]): HandRank {
    ...
    return {
      rank: 1,
      description: "하이카드",
      cards: highCards, // Return all 5 cards
      kickers: []
    };
  }
